...
### Đã kiểm chứng trên máy này (2026-07-22)

Không còn là phỏng đoán. Một lượt khảo sát đã chạy thật và cho kết quả:

| Câu hỏi | Kết quả |
|---|---|
| `core/` build trên Linux? | **Có**, GCC 14.2, C++20, zero warning. `gpu/` gated WIN32 đúng |
| Test? | **13/13 pass** (repo ghi 17 — 4 cái kia nằm trong `gpu/`, không tồn tại trên Linux) |
| Huấn luyện chạy được? | **Có**, ~790 steps/giây trên CPU. Reward chưa hội tụ sau 30 episode — đúng như dự kiến, `epsilon_decay_steps=20000` nên agent vẫn còn ~50% ngẫu nhiên |
| Có `IEnvironment` không? | **Không** — và đó là tin tốt. `ITrainerBackend`, `Experience`, `ReplayBuffer` hoàn toàn không biết môi trường. Chỉ `DqnTrainer` (20 dòng) gắn cứng vào `CarEnvironment`. **Cách thay: không dùng `DqnTrainer`, tự viết vòng lặp.** Không kế thừa, không interface phải implement |
| Observation / action / reward | `std::vector<float>` độ dài = `input_size` (tự chuẩn hoá) / **một `int`** rời rạc / `float` tuỳ ý |

### ⚠ Một cạm bẫy phải xử lý trước khi viết dòng code nào

`core/src/rl/DqnAgent.cpp:25` gắn cứng `kActionCount = 15` (5 lái × 3 ga của **xe hơi**), **không**
lấy từ `NetworkSpec::output_size`:

```cpp
std::uniform_int_distribution<int> action_dist(0, kActionCount - 1);
```

Nếu không gian hành động của ta **ít hơn 15**, epsilon-greedy trả về chỉ số ngoài phạm vi, rồi
`CpuTrainerBackend::TrainBatch` ghi vào đó **không kiểm tra biên**. Đã tái hiện được với môi trường
4 hành động — **segfault ngay**, ASan xác nhận heap-buffer-overflow.

Đây là lỗi tiềm ẩn trong repo gốc (số hành động của họ tình cờ khớp), nhưng nó sẽ cắn ta ngay lập
tức. Hai lựa chọn: thiết kế đúng 15 hành động rời rạc (ví dụ 5 di chuyển × 3 chế độ đánh — chạy
được, không sửa gì), hoặc thêm `action_count` vào `DqnAgentParams`. `gpu/src/GpuDqnTrainer.cpp:77`
có y hệt chỗ gắn cứng đó.

### Boss không được ngu lúc mới sinh

Điểm này đúng và quan trọng: **mạng khởi tạo ngẫu nhiên nghĩa là boss sinh ra hành xử ngẫu nhiên.**
Không chấp nhận được — người chơi gặp boss lần đầu mà nó đứng ngơ thì cả tính năng RL thành trò cười.

Cách xử lý, đã kiểm chứng round-trip **bit-exact**:

1. Huấn luyện **offline** (không phải trong game), lưu `NetworkCheckpoint` — JSON, 1743 trọng số =
   **24 KB**. Commit thẳng vào repo, human-readable và diff được.
2. Lúc spawn: nạp checkpoint → `unflatten` vào mạng → **`PredictOnline(obs)` + argmax**. Bỏ qua hẳn
   `DqnAgent` nếu boss không cần học tiếp trong game — đường suy luận này rẻ hơn, không cấp phát
   replay buffer.
3. "Thế hệ 0" của mọi hầm ngục là checkpoint đã pre-train này, không phải trọng số ngẫu nhiên.

**Phải tự viết ~10 dòng `flatten`/`unflatten`**: `weights` là vector phẳng, nhưng helper duy nhất
(`NetworkGpuLayout::FlattenWeights`) nằm trong `gpu/` chỉ có trên Windows. Layout đã xác nhận: mỗi
lớp `[weights: out×in row-major][biases: out]`, các lớp theo thứ tự.
